...
public class ViewData extends Activity {
    ...
    public void exportCSV(String filename) {
        ...
        if(usernameSpinner.getSelectedItemPosition() != 0 )
            data.addAll(mydb.getAllRegistsByUsername(usernameSpinner.getSelectedItem().toString().trim()));
        else
        if(dateSpinner.getSelectedItemPosition() != 0 )
            data.addAll(mydb.getAllRegistsByDate(dateSpinner.getSelectedItem().toString().trim()));
        else
        if(movenameSpinner.getSelectedItemPosition() != 0 )
            data.addAll(mydb.getAllRegistsByMovename(movenameSpinner.getSelectedItem().toString().trim()));
        else
            data.addAll(mydb.getAllRegists());
        ...
        try {
            ...
            if( !myFile.exists() ) {
                ...
                fw.append("MoveId; Timestamp; Accel_x; Accel_y; Accel_z; Gyro_x; Gyro_y; Gyro_z; Orient_x; Orient_y; Orient_z; Orient_w; CurrentArm; Reference; Username; MoveName\n");
                fw.close();
            }
            FileWriter fw  = new FileWriter(myFile, true);
